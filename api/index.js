...
app.post("/register", async (req, res) => {
    ...
    if (!username || !email || !password) {
        return res
            .status(400)
            .send("Bad request: empty username, email or password")
    }
    ...
    try {
        ...
        console.log(
            `New user ${username}, ${email} has been successfully created!`,
        )
        // Open session
...
function openSession(ip, username, email, rememberMe) {
    ...
    if (!rememberMe) {
        ...
        const timeoutId = setTimeout(() => {
            console.log(
                `Session timeout for ${ip} and ${email}. Closing session`,
            )
    ...
    console.log(
        `New session ${username}, ${email} from ${ip} has been successfully opened!`,
    )
}
...
function closeAllSessions(email) {
    ...
    if (openedSessions.length < initialLength) {
        console.log(
            `All sessions for email ${email} have been successfully closed!`,
        )
        return true
    } else {
...
function closeSession(ip, email) {
    const initialLength = openedSessions.length
    openedSessions = openedSessions.filter(
        (session) => !(session.email === email && session.ip === ip),
    )

    if (openedSessions.length < initialLength) {
        console.log(
            `A session for email ${email} with IP ${ip} has been successfully closed!`,
        )
        return true
    } else {
        console.warn(
            `No active sessions were found for email ${email} with IP ${ip}.`,
        )
        return false
    }
}
...
function clearAllSessionTimers(email) {
    openedSessions.forEach((session) => {
        if (session.email === email && session.timeoutId) {
            ...
            session.timeoutId = null
            console.log(
                `Timeout cleared for session ${email} from IP ${session.ip}`,
            )
        }
    })
}
